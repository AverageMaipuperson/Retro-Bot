...
bool Toggler::init(const char* text, bool* var, CCObject* target, SEL_MenuHandler selector, int tag) {
        m_opt = var;
        ...
        m_button = CCMenuItemToggler::create(off, on, target, selector);
        m_button->setTag(tag);
        m_button->setUserData(var);
        if (var && *var) {
        m_button->toggle(true); 
    }
